...
(() => {
/*!*************************************************!*\
  !*** ./resources/js/backoffice/utilizadores.js ***!
  \*************************************************/
/* provided dependency */ var $ = __webpack_require__(/*! jquery */ "./node_modules/jquery/dist/jquery.js");
var _require = __webpack_require__(/*! axios */ "./node_modules/axios/index.js"),
    axios = _require["default"];

var DataTable = __webpack_require__(/*! datatables.net */ "./node_modules/datatables.net/js/jquery.dataTables.js"); //Modal
...
document.getElementById('createUserModal').addEventListener('click', function () {
  ...
  axios.get('/users/getTiposUser').then(function (response) {
    console.log(response);
    var radios = modalUser.input.tipo;

    for (var i = 0; i < response.data.length; i++) {
      radios.innerHTML += "<div class=\"ms-5\"><input id=\"radio_".concat(response.data[i].id, "\" name=\"radio\" class=\"form-check-input radioTipo\" type=\"radio\" value=\"").concat(response.data[i].id, "\" />\n                                    <label class=\"form-check-label\" for=\"radio_").concat(response.data[i].id, "\">").concat(response.data[i].tipo, "</label></div>");
    }
  })["catch"](function (error) {
    console.log(error);
  });
...
var editarUserBtns = document.querySelectorAll('.btedituser');
editarUserBtns.forEach(function (elemento) {
  elemento.addEventListener('click', function (evento) {
    ...
    modalUser.btn.guardar.style.display = 'inline-block';
    modalUser.btn.transferir.style.display = 'none';
    axios.get("/getuser/".concat(id), {}).then(function (response) {
      console.log(response.data);
      modalUser.input.nome.value = response.data[0];
      modalUser.input.email.value = response.data[1];
      var usertipo = response.data[2];
      var tipos = response.data[3];
      var radios = modalUser.input.tipo;

      for (var i = 0; i < tipos.length; i++) {
        radios.innerHTML += "<div class=\"ms-5\"><input id=\"radio_".concat(tipos[i].id, "\" name=\"radio\" class=\"form-check-input radioTipo\" type=\"radio\" value=\"").concat(tipos[i].id, "\" />\n                                    <label class=\"form-check-label\" for=\"radio_").concat(tipos[i].id, "\">").concat(tipos[i].tipo, "</label></div>");
      }
...
var userNotAuthBtns = document.querySelectorAll('.btnUserNotAuth');
userNotAuthBtns.forEach(function (elemento) {
  elemento.addEventListener('click', function (evento) {
    modalUser.clear();
    var id = elemento.getAttribute('id').split('_').pop();
    modalUser.labelIdUser.setAttribute('userId', id);
    modalUser.titulo.innerText = '';
    modalUser.titulo.innerText = 'User Não Autenticado'; //retirar campos desnecessários
    ...
    modalUser.btn.guardar.style.display = 'none'; //adicionar botao

    modalUser.btn.transferir.style.display = 'inline-block';
    axios.get("/getusernotauth/".concat(id), {}).then(function (response) {
  ...
  $('#tableUsers').DataTable({
    ...
    initComplete: function initComplete() {
      this.api().columns([0, 1, 2, 3]).every(function (d) {
        ...
        var select = $('<select class="form-control my-1 fw-bold"><option value="">' + theadname + '</option></select>').appendTo($(column.header()).empty()).on('change', function () {
...
modalUser.btn.guardar.addEventListener('click', function () {
  var username = modalUser.input.nome;
  var email = modalUser.input.email;
  var pass = modalUser.input.pass;
  var userId;
  ...
  var tipo;
  var radios = document.querySelectorAll('.radioTipo');
...
modalUser.btn.transferir.addEventListener('click', function () {
  var username = modalUser.input.nome;
  var email = modalUser.input.email;
  var pass = '999999999';
  var tipo = 1;
  var userId = null;
  var userNotAuth = true;
  axios.post('/users/save', {
    'username': username.value,
    'email': email.value,
    'pass': pass,
